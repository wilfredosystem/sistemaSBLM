...
@ManagedBean(name = "arrendamientosupervisionMB")
@ViewScoped
public class ArrendamientoSupervisionManagedBean implements Serializable {
	
	@ManagedProperty(value = "#{arrendamientoSupervisionService}")
	private transient IArrendamientoSupervisionService arrendamientoSupervisionService;
	...
	public void ObjectsInitialization(){
		todosUsuarios=arrendamientoSupervisionService.obtenerUsuarios();
		...
		listaPerfiles= arrendamientoSupervisionService.obtenerPerfiles();
		
	}
	
	public Boolean grantedAccess(){
		
		if(FuncionesHelper.nombreperfilSeleccionado.equals("Supervisor Arrendamiento")){ 
			visible=true;
			System.out.println("GRANTED ACCESS - ARRENDAMIENTO SUPERVISOR");
		}else {
			visible=false;
			System.out.println("DENIED ACCESS - ARRENDAMIENTO SUPERVISOR");
		}
			return visible;
	}
	...
	public void buscarPerfilUsuario(){
		...
		perfilesusuario=arrendamientoSupervisionService.getPerfilesUsuario(getIdUsuarioSeleccionado());
	...
	public void registrarSeguimientoFlujo(){
		...
		for (int i = 0; i < documentosSeleccionadosDerivacion.size(); i++) {
			arrendamientoSupervisionService.actualizarFlujoDocumentoSupervisor(documentosSeleccionadosDerivacion.get(i).getIdflujodocumento());
			...
			for (Usuario user : listadousuariosSeleccionados) {
				...
				arrendamientoSupervisionService.registrarSeguimientoFlujo(segflujo,user.getIdusuario());
			}
    ...
    public void obtenerSeguimientoFlujodeDocumento(){
    	...
    	if(documentosSeleccionadosAtendidos.size()!=0){
    		...
    		seguimientoFlujoDocumento=arrendamientoSupervisionService.obtenerSeguimientoFlujodeDocumento(idflujoDocumento);
	...
	public void notificarRechazo(){
		if(getComentarioRechazo()!=null){
			arrendamientoSupervisionService.actualizarComentarioRespuestaFlujoDocumento(documentosSeleccionadosAtendidos.get(0).getIdflujodocumento(), comentarioRechazo,seguimientoFlujoDocumento);
	 ...
	 public void regresarDGAI(){
			...
			for(int i=0;i<documentosSeleccionadosAtendidos.size();i++){
				arrendamientoSupervisionService.regresarDGAI(documentosSeleccionadosAtendidos.get(i).getIdflujodocumento());
			}
		...
		public void contadoresEstadoGeneral(){
			numeroAprobadosMes=arrendamientoSupervisionService.obtenerNumeroDocumentosMes("APROBADO");
			numeroProcesoMes=arrendamientoSupervisionService.obtenerNumeroDocumentosMes("EN PROCESO");
			numeroRechazadosMes=arrendamientoSupervisionService.obtenerNumeroDocumentosMes("RECHAZADO");
		}
		
		public void contadoresEstadoMensual(){
			numeroAprobadosMes=arrendamientoSupervisionService.obtenerNumeroDocumentosMes("APROBADO",mesActualcapturado);
			numeroProcesoMes=arrendamientoSupervisionService.obtenerNumeroDocumentosMes("EN PROCESO",mesActualcapturado);
			numeroRechazadosMes=arrendamientoSupervisionService.obtenerNumeroDocumentosMes("RECHAZADO",mesActualcapturado);
		}
		
		public void contadoresGeneral(){
			numeroPendientesMes=arrendamientoSupervisionService.obtenerNumeroPendientes();
			numeroDerivadosMes=arrendamientoSupervisionService.obtenerNumeroDerivados();
		}
		 
		public void contadoresMensual(){
			numeroPendientesMes=arrendamientoSupervisionService.obtenerNumeroPendientesMes(mesActualcapturado);
			numeroDerivadosMes=arrendamientoSupervisionService.obtenerNumeroDerivadosMes(mesActualcapturado);
		}
		
		//FILTRADO POR MES
		
		public void filtrarAprobadosMes(){
			listFLujoDocumentosSupervisorAtendido=arrendamientoSupervisionService.listaFiltroDocumentosMes("APROBADO");
		}
		public void filtrarRechazadosMes(){
			listFLujoDocumentosSupervisorAtendido=arrendamientoSupervisionService.listaFiltroDocumentosMes("RECHAZADO");
		}
		public void filtrarProcesoMes(){
			listFLujoDocumentosSupervisorAtendido=arrendamientoSupervisionService.listaFiltroDocumentosMes("EN PROCESO");
		}
	...
	public void verificarExistenciaSeguimientoFlujoRechazo(){
			...
			if(documentosSeleccionadosAtendidos.size()==1){
				int tamanioFlujoDocumento=arrendamientoSupervisionService.obtenertamanioFlujoDocumento(documentosSeleccionadosAtendidos.get(0).getIdflujodocumento());
				Boolean finalizadoDocumento=arrendamientoSupervisionService.estaDocumentoFinalizado(documentosSeleccionadosAtendidos.get(0).getIdflujodocumento());
				Boolean finalizadoFlujo=arrendamientoSupervisionService.estaFlujoFinalizado(documentosSeleccionadosAtendidos.get(0).getIdflujodocumento());
		...
		public void verificarExistenciaSeguimientpoFlujoDespacho(){
			...
			if(documentosSeleccionadosAtendidos.size()!=0){
				int tamanioFlujoDocumento=arrendamientoSupervisionService.obtenertamanioFlujoDocumento(documentosSeleccionadosAtendidos.get(0).getIdflujodocumento());
				Boolean finalizado=arrendamientoSupervisionService.estaDocumentoFinalizado(documentosSeleccionadosAtendidos.get(0).getIdflujodocumento());
				Boolean finalizadoFlujo=arrendamientoSupervisionService.estaFlujoFinalizado(documentosSeleccionadosAtendidos.get(0).getIdflujodocumento());
	...
	public void enviarNotificaciónPersonalizable(){
		...
		arrendamientoSupervisionService.enviarNotificaciónPersonalizable(getContenidoMensajePersonalizado(),getIdusuarioNotificacionPersonalizada());
	...
	public void getFlujoDocumentosSupervisor(int mesSeleccionado){
		listFLujoDocumentosSupervisorPendiente=arrendamientoSupervisionService.getFlujoDocumentosSupervisor(mesSeleccionado);
	}
	
	
	
	public void getFlujoDocumentosSupervisorAtendidos(int mesSeleccionado){
		listFLujoDocumentosSupervisorAtendido=arrendamientoSupervisionService.getFlujoDocumentosSupervisorAtendidos(mesSeleccionado);
	}
	...
	public IArrendamientoSupervisionService getArrendamientoSupervisionService() {
		return arrendamientoSupervisionService;
	}

	
	public void setArrendamientoSupervisionService(
			IArrendamientoSupervisionService arrendamientoSupervisionService) {
		this.arrendamientoSupervisionService = arrendamientoSupervisionService;
	}
