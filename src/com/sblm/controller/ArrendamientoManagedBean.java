...
@ManagedBean(name = "arrendamientoMB")	
@ViewScoped
public class ArrendamientoManagedBean implements Serializable {
	
	private static final long serialVersionUID = 1L;

	@ManagedProperty(value = "#{arrendamientoService}")
	private transient IArrendamientoService arrendamientoService;
	...
	public Boolean grantedAccess(){
		...
		if(FuncionesHelper.nombreperfilSeleccionado.equals("Arrendamiento")){
			visible=true;
			System.out.println("GRANTED ACCESS - ARRENDAMIENTO");
		}else {
			visible=false;
			System.out.println("DENIED ACCESS - ARRENDAMIENTO");
     	}
		return visible;
	...
	public void cargarArchivosDocumento(int iddoc){
	 ...
	 public void regresarSupervisor(){
		...
		for(int i=0;i<documentosSeleccionadosPendientes.size();i++){
			arrendamientoService.regresarSupervisor(documentosSeleccionadosPendientes.get(i).getIdsegflujodocumento());
		}
	 ...
	 public void notificarRechazo(){
		if(getComentarioRechazo()!=null){
			arrendamientoService.actualizarComentarioRespuestaSeguimientoFlujoDocumento(documentosSeleccionadosPendientes.get(0).getIdsegflujodocumento(), comentarioRechazo,documentosSeleccionadosPendientes.get(0).getIdusuarioremitente());
	...
	public void verificarFinalizadoRechazo(){
		...
		if(documentosSeleccionadosPendientes.size()==1){
			Boolean finalizadoDocumento=arrendamientoService.estaDocumentoFinalizado(documentosSeleccionadosPendientes.get(0).getIdsegflujodocumento());
	...
	private void getSeguimientoFlujoDocumentosAtendidos(int mesSeleccionado) {
		if(visible){
		listSeguimientoFLujoDocumentosAutovaluoAtendido=arrendamientoService.obtenerSeguimientoFlujodeDocumentoAtendido(mesSeleccionado);
		}else {
	...
	public void getSeguimientoFlujoDocumentosPendientes(int mesSeleccionado){
		if(visible){
			listSeguimientoFLujoDocumentosAutovaluoPendiente=arrendamientoService.obtenerSeguimientoFlujodeDocumento(mesSeleccionado);
		}
	...
	public void contadoresEstadoGeneral(){
		if(visible){
		numeroAprobadosMes=arrendamientoService.obtenerNumeroDocumentosMes("APROBADO");
		numeroProcesoMes=arrendamientoService.obtenerNumeroDocumentosMes("EN PROCESO");
		numeroRechazadosMes=arrendamientoService.obtenerNumeroDocumentosMes("RECHAZADO");
		}
		}
	
	public void contadoresEstadoMensual(){
		numeroAprobadosMes=arrendamientoService.obtenerNumeroDocumentosMes("APROBADO",mesActualcapturado);
		numeroProcesoMes=arrendamientoService.obtenerNumeroDocumentosMes("EN PROCESO",mesActualcapturado);
		numeroRechazadosMes=arrendamientoService.obtenerNumeroDocumentosMes("RECHAZADO",mesActualcapturado);
	}
	
	public void contadoresGeneral(){
		if(visible){
		numeroPendientesMes=arrendamientoService.obtenerNumeroPendientes();
		numeroDerivadosMes=arrendamientoService.obtenerNumeroDerivados();
		}
	}
	 
	public void contadoresMensual(){
		numeroPendientesMes=arrendamientoService.obtenerNumeroPendientesMes(mesActualcapturado);
		numeroDerivadosMes=arrendamientoService.obtenerNumeroDerivadosMes(mesActualcapturado);
	}
	...
	public void filtrarAprobadosMes(){
		listSeguimientoFLujoDocumentosAutovaluoAtendido=arrendamientoService.listaFiltroDocumentosMes("APROBADO");
		
	}
	public void filtrarRechazadosMes(){
		listSeguimientoFLujoDocumentosAutovaluoAtendido=arrendamientoService.listaFiltroDocumentosMes("RECHAZADO");
	}
	public void filtrarProcesoMes(){
		listSeguimientoFLujoDocumentosAutovaluoAtendido=arrendamientoService.listaFiltroDocumentosMes("EN PROCESO");
	}
	...
	public IArrendamientoService getArrendamientoService() {
		return arrendamientoService;
	}

	public void setArrendamientoService(IArrendamientoService arrendamientoService) {
