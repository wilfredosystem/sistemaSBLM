...
@ManagedBean(name = "recaudacionautovaluoMB")
@ViewScoped
public class RecaudacionAutovaluoManagedBean implements Serializable {
	
	private static final long serialVersionUID = 1L;

	@ManagedProperty(value = "#{recaudacionAutovaluoService}")
	private transient IRecaudacionAutovaluoService recaudacionAutovaluoService;
	...
	public Boolean grantedAccess(){
		...
		if(FuncionesHelper.nombreperfilSeleccionado.equals("Autovalúo")){
			visible=true;
			System.out.println("GRANTED ACCESS - AUTOVALUO");
		}else {
			visible=false;
			System.out.println("DENIED ACCESS - AUTOVALUO");
		}
	...
	public void cargarArchivosDocumento(int iddoc){
	 ...
	 public void regresarSupervisor(){
			...
			for(int i=0;i<documentosSeleccionadosPendientes.size();i++){
				recaudacionAutovaluoService.regresarSupervisor(documentosSeleccionadosPendientes.get(i).getIdsegflujodocumento());
			}
	 ...
	 public void notificarRechazo(){
			if(getComentarioRechazo()!=null){
				recaudacionAutovaluoService.actualizarComentarioRespuestaSeguimientoFlujoDocumento(documentosSeleccionadosPendientes.get(0).getIdsegflujodocumento(), comentarioRechazo,documentosSeleccionadosPendientes.get(0).getIdusuarioremitente());
	...
	public void verificarFinalizadoRechazo(){
		...
		if(documentosSeleccionadosPendientes.size()==1){
			Boolean finalizadoDocumento=recaudacionAutovaluoService.estaDocumentoFinalizado(documentosSeleccionadosPendientes.get(0).getIdsegflujodocumento());
	...
	private void getSeguimientoFlujoDocumentosAtendidos(int mesSeleccionado) {
		if(visible){
		listSeguimientoFLujoDocumentosAutovaluoAtendido=recaudacionAutovaluoService.obtenerSeguimientoFlujodeDocumentoAtendido(mesSeleccionado);
		}
	}
	...
	public void getSeguimientoFlujoDocumentosPendientes(int mesSeleccionado){
		if(visible){
			listSeguimientoFLujoDocumentosAutovaluoPendiente=recaudacionAutovaluoService.obtenerSeguimientoFlujodeDocumento(mesSeleccionado);
		}
	...
	public void contadoresEstadoGeneral(){
		if(visible){
		numeroAprobadosMes=recaudacionAutovaluoService.obtenerNumeroDocumentosMes("APROBADO");
		numeroProcesoMes=recaudacionAutovaluoService.obtenerNumeroDocumentosMes("EN PROCESO");
		numeroRechazadosMes=recaudacionAutovaluoService.obtenerNumeroDocumentosMes("RECHAZADO");
		}
		}
	
	public void contadoresEstadoMensual(){
		numeroAprobadosMes=recaudacionAutovaluoService.obtenerNumeroDocumentosMes("APROBADO",mesActualcapturado);
		numeroProcesoMes=recaudacionAutovaluoService.obtenerNumeroDocumentosMes("EN PROCESO",mesActualcapturado);
		numeroRechazadosMes=recaudacionAutovaluoService.obtenerNumeroDocumentosMes("RECHAZADO",mesActualcapturado);
	}
	
	public void contadoresGeneral(){
		if(visible){
		numeroPendientesMes=recaudacionAutovaluoService.obtenerNumeroPendientes();
		numeroDerivadosMes=recaudacionAutovaluoService.obtenerNumeroDerivados();
		}
	}
	 
	public void contadoresMensual(){
		numeroPendientesMes=recaudacionAutovaluoService.obtenerNumeroPendientesMes(mesActualcapturado);
		numeroDerivadosMes=recaudacionAutovaluoService.obtenerNumeroDerivadosMes(mesActualcapturado);
	}
	...
	public void filtrarAprobadosMes(){
		listSeguimientoFLujoDocumentosAutovaluoAtendido=recaudacionAutovaluoService.listaFiltroDocumentosMes("APROBADO");
		
	}
	public void filtrarRechazadosMes(){
		listSeguimientoFLujoDocumentosAutovaluoAtendido=recaudacionAutovaluoService.listaFiltroDocumentosMes("RECHAZADO");
	}
	public void filtrarProcesoMes(){
		listSeguimientoFLujoDocumentosAutovaluoAtendido=recaudacionAutovaluoService.listaFiltroDocumentosMes("EN PROCESO");
	}
	

	public IRecaudacionAutovaluoService getRecaudacionAutovaluoService() {
		return recaudacionAutovaluoService;
	}

	public void setRecaudacionAutovaluoService(
			IRecaudacionAutovaluoService recaudacionAutovaluoService) {
		this.recaudacionAutovaluoService = recaudacionAutovaluoService;
	}
